...
const server = http.createServer((req, res) =>{
    ...
    if(req.url ==='/'){ // home page
        ...
        const headers =
        {
            "Access-Control-Allow-Origin": "*", // To allow data btween different platforms
            "Access-Control-Allow-Methods": "OPTIONS, POST, GET",
            "Content-Type": 'application/json' //Response content will be in JSON format
        };
        (async(req,res)=>{
            try
            {
                ...
                console.log(wonders_collection);
                const cursor = wonders_collection.find({});
                const results = await cursor.toArray();
                const js = (JSON.stringify(results));
                res.writeHead(200, headers);
                res.end(js); //sends the response to the client and ends the request-response cycle.
                console.log(js);
            }catch (err) {
                console.error(err);
